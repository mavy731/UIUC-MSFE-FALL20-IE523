...
class Towers 
{
	...
	{ 
		...
		bool result = true;
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < towers.at(i).size(); j++)
				for (int k = j; k < towers.at(i).size(); k++)
					if (towers.at(i).at(k) < towers.at(i).at(j)) {
						result = false;
					}
		return (result);
	...
	void move_top_disk(int source, int dest)
	{
		// write the code for this part
		int disk;
		disk = towers.at(source).at(0);
		towers.at(dest).push_front(disk);
		towers.at(source).pop_front();
		if (is_everything_legal()) {
			cout << "Move disk " << disk << " from Peg " << source + 1 << " to Peg " << dest + 1 << " (Legal)" << endl;
		}
		else {
			cout << "Move disk " << disk << " from Peg " << source + 1 << " to Peg " << dest + 1 << " (Illegal)" << endl;
		}
	}
	...
	void Move(int number_of_disks, int source_peg_number, int destination_peg_number, deque <int> free_peg_numbers)
	{
		if (free_peg_numbers.size() != 0)
		{
			if (number_of_disks > 1) 
			{
				...
				// write the code for this part
				int pegi;
				pegi = free_peg_numbers.at(0);
				free_peg_numbers.pop_front();
				free_peg_numbers.push_front(destination_peg_number);
				Move(m, source_peg_number, pegi, free_peg_numbers);
				free_peg_numbers.pop_front();
				Move(number_of_disks - m, source_peg_number, destination_peg_number, free_peg_numbers);
				free_peg_numbers.push_front(source_peg_number);
				Move(m, pegi, destination_peg_number, free_peg_numbers);
			}
			else {
				// number_of_disks == 1
				number_of_steps++;
	...
	void print_peg_state(int m)
	{
		...
		for (int i = 0; i < towers.at(m).size(); i++) {
			cout << towers.at(m).at(i) << " ";
		}
		cout << endl;
