...
double Simulation_Call, Simulation_Put, BB_Call, BB_Put;
...
void simulation_european(int T, double r, double sigma, double S_0, double K, int n, int m, double B) {
	...
	for (int i = 0;i < n;i++) {
		...
		//Calculate Brownian-Bridge Prob.
		for (int k = 1;k < 5;k++) {
			if (S[k] > 0) {
				for (int j = 1;j < m;j++) {
					double mu, var;
					mu = S_0 + (d_t * j / T) * (S[k] - S_0);
					var = (d_t * j * (T - d_t * j) )/ T;
					p = p * (1 - N((B - mu) / sqrt(var)));
				}
	...
	BB_Call = adj_sum_call / (count * exp(r * T));
	BB_Put = adj_sum_put / (count * exp(r * T));
}
...
int main(int argc, char* argv[])
{
	...
	cout << "European Down-and-Out Discrete Barrier Options Pricing" << endl;
	...
	cout << "The average Call Price with Brownian-Bridge correction on the final price = " << BB_Call << endl;
	cout << "The average Put Price by explicit simulation                             = " << Simulation_Put << endl;
	cout << "The average Put Price with Brownian-Bridge correction on the final price = " << BB_Put << endl;
	cout << "--------------------------------------" << endl;
