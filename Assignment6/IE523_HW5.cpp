...
void create_solution_vector(Matrix A, ColumnVector y)
{
...

	{
		...
		do 
		{
			...
			vector <double> B_initial_rows(A.nrows(), 0);
			vector <vector <double> > B(A.ncols(),B_initial_rows);
            // Look at your solution the 1st programming assignment
			for (int i = 0, Bcol=0; i < A.ncols(); ++i) 
			{
			...
			if (Rank(Column_reduced_version_of_A) == rank_of_A)
			{
                ...
                // vector-of-vectors...
				vector <int> zero_position (A.nrows(),0);

				for (int c = 0; c < A.ncols();c++) {
					if (!did_i_pick_this_col[c]) {
						zero_position[c] = 1;
					}
				}
				for (int m = 0, n = 1; m < A.ncols();m++) {
					if (zero_position[m]==1) {
						temp.push_back(0);
					}
					else {
						temp.push_back(soln(n));
						n++;
					}
				}
...
void trim_the_solution_set(Matrix A, ColumnVector y)
{
	...
	
	{
		...
		do 
		{
			vector <double> sol_initial_rows(solution.nrows(), 0);
			vector <vector <double> > B(solution.ncols(), sol_initial_rows);
			
			// write code that picks (rank_of_solution_set)-many columns
            // of the local-matrix "solution" and stores these columns in B
			for (int c = 0, Bc=0;c < solution.nrows();c++) {
				if (did_i_pick_this[c]) {
					for (int r = 0; r < solution.nrows();r++) {
						B[Bc][r] = solution(r + 1, c + 1);
					}
					Bc++;
				}
			}
